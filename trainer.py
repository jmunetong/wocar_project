...
from policy_gradients.auto_LiRPA import BoundedModule
from policy_gradients.auto_LiRPA.eps_scheduler import LinearScheduler
from policy_gradients.auto_LiRPA.bounded_tensor import BoundedTensor
from policy_gradients.auto_LiRPA.perturbations import PerturbationLpNorm
from policy_gradients.models import *
from policy_gradients.torch_utils import *
from policy_gradients.steps import value_step, step_with_mode
from policy_gradients.worst_q import worst_q_step
from log import *
from policy_gradients.custom_env import Env
from policy_gradients.step_utils import *
...
class Trainer():
    ...
    def __init__(self, policy_net_class, value_net_class, params,
        ...
        # Minimax training
        if self.MODE == 'adv_ppo' or self.MODE == 'adv_sa_ppo':
        ...
        print(f'VALUE ACTIVATION: {self.value_activation}')
    ...
    def advantage_and_return(self, rewards, values, not_dones):
        ...
        V_s_tp1 = torch.cat([values[:,1:], values[:, -1:]], 1) * not_dones
    ...
    def run_trajectories(self, num_saps, return_rewards=False, should_tqdm=False,
        ...
        # action_stds = torch.zeros(actions_shape)
        ...
        is_advpolicy_training = self.MODE == "adv_ppo"  or self.MODE == "adv_sa_ppo"
        ...
        for t in iterator:
            ...
            if is_advpolicy_training:
                ...
                if collect_adversary_trajectory or self.params.ADV_ADVERSARY_RATIO >= random.random():
                    ...
                    last_states = last_states + torch.nn.functional.hardtanh(next_adv_perturbations) * self.ADV_EPS
    ...
    def apply_attack(self, last_states):
        ...
        if self.params.ATTACK_METHOD == "critic":
            ...
            # Clamp using tanh.
            perturbed_states = last_states + torch.nn.functional.hardtanh(perturbations_mean) * eps
            """
            ...
            perturbed_states = last_states + torch.tanh(next_adv_perturbations) * eps
            """
    ...
    def sarsa_steps(self, saps):
        ...
        # We treat all value epochs as one epotorch.
    ...
    def take_steps(self, saps, logging=True, value_only=False, adversary_step=False, increment_scheduler=True):
        ...
        if adversary_step:
            ...
            if self.MODE == 'robust_q_ppo': 
                policy_loss, surr_loss, entropy_bonus, data = self.policy_step(*args, target_q_net=self.target_q_model, value_net=self.val_model, n_steps = self.n_steps)
            else:
                policy_loss, surr_loss, entropy_bonus, data = self.policy_step(*args)
        ...
        if self.params.MODE == 'robust_q_ppo':
            ...
            return policy_loss, surr_loss, entropy_bonus, val_loss, q_loss, data

        if self.ANNEAL_LR:
            print(f'val lr: {val_scheduler.get_last_lr()}, policy lr: {policy_scheduler.get_last_lr()}')
        val_loss = val_loss.mean().item()
        return policy_loss, surr_loss, entropy_bonus, val_loss, data
        ...
        if self.MODE == "adv_ppo" or self.MODE == "adv_sa_ppo":
            ...
            if self.PPO_LR_ADAM != 0.0:
                for i in range(int(self.ADV_POLICY_STEPS)):
                    avg_ep_reward,d  = self.train_step_impl(adversary_step = False, increment_scheduler = (i==self.ADV_POLICY_STEPS-1))
                ...
                avg_ep_reward, d = self.train_step_impl(adversary_step = True)
        else:
            avg_ep_reward, d = self.train_step_impl(adversary_step = False)

        self.n_steps += 1
        return avg_ep_reward, d
    ...
    def train_step_impl(self, adversary_step=False, increment_scheduler=True):
        ...
        if self.params.MODE == 'robust_q_ppo':
            policy_loss, surr_loss, entropy_bonus, val_loss, q_loss, data = self.take_steps(saps, adversary_step=adversary_step, increment_scheduler=increment_scheduler)
        else:
            policy_loss, surr_loss, entropy_bonus, val_loss, data = self.take_steps(saps, adversary_step=adversary_step, increment_scheduler=increment_scheduler)
        # Logging code
        ...
        if not policy_model.discrete:
            mean_std = torch.exp(policy_model.log_stdev).mean()
    ...
    def agent_from_params(params, store=None):
        ...
        return p, params['value_net_type']
