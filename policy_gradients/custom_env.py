...
from policy_gradients.torch_utils import ZFilter, Identity, StateWithTime, RewardFilter
...
class Env:
    ...
    def __init__(self, game, norm_states, norm_rewards, params, add_t_with_horizon=None, clip_obs=None, clip_rew=None, 
        ...
        if isinstance(self.env.observation_space, gym.spaces.Box):
            action_space = action_space = self.env.action_space

        # Check if the space is Discrete to get the number of actions
        if isinstance(self.env.action_space, gym.spaces.Discrete):
            action_space = self.env.action_space
        
        self.action_shape = action_space.shape
        self.is_discrete = type(self.env.action_space) == gym.spaces.Discrete
        assert self.is_discrete or type(self.env.action_space) == gym.spaces.Box
        assert len(self.action_shape) <= 1 # scalar or vector actions
        self.num_actions = action_space.n if self.is_discrete else 0 \
                            if len(self.action_shape) == 0 else self.action_shape[0]
