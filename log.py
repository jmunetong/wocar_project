...
from  policy_gradients.value import value_loss_returns, value_loss_gae
...
def log_weight_updates(agent, old_params, new_params, table='weight_updates'):
    ...
    for name in old_params.keys():
        ...
        diff_w = new_w - old_w
        l1 = torch.norm(new_w.view(-1), p=1).item()
        l2 = torch.norm(new_w.view(-1), p=2).item()
        linf = torch.norm(new_w.view(-1), p=np.inf).item()
        l1_delta = torch.norm(diff_w.view(-1), p=1).item()
        l2_delta = torch.norm(diff_w.view(-1), p=2).item()
        linf_delta = torch.norm(diff_w.view(-1), p=np.inf).item()
...
def paper_constraints_logging(agent, saps, old_pds, table):
    ...
    ratios = torch.exp(new_log_ps - saps.action_log_probs)
    max_rat = ratios.max()
